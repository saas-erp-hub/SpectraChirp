...
def mock_external_dependencies():
    with (
        ...
        # Configure mocks
        mock_sd.rec.return_value = np.ones(
            SAMPLE_RATE * 5
        )  # 5 seconds of non-zero signal
...
def test_send_live_mode():
    with patch("cli.typer.secho") as mock_secho, patch("cli.sd.play") as mock_sd_play:
...
def test_send_expert_mode_success():
    ...
    with (
        patch("cli.typer.secho") as mock_secho,
...
def test_send_error_file_not_found():
    with (
        ...
        assert result.exit_code == 1
        mock_secho.assert_any_call(
            "Error: Input file not found at 'nonexistent.txt'", fg=typer.colors.RED
        )
...
def test_send_error_playback_failure():
    with (
        ...
        assert result.exit_code == 1
        mock_secho.assert_any_call(
            "Error playing audio: Playback error", fg=typer.colors.RED
        )
...
def test_send_error_file_write_failure():
    ...
    with (
        ...
        assert result.exit_code == 1
        mock_secho.assert_any_call(
            "Error writing file: Write error", fg=typer.colors.RED
        )
...
def test_receive_from_file_success():
    with (
        patch("cli.typer.secho") as mock_secho,
        patch(
            "cli.receive_text_mfsk",
            return_value=("decoded text", None, None, "DEFAULT"),
        ),
    ):
...
def test_receive_from_file_and_save_to_file():
    with (
        ...
        mock_secho.assert_any_call(
            "Decoded message saved to 'output.txt'", fg=typer.colors.GREEN
        )
...
def test_receive_live_success():
    with (
        ...
        patch(
            "cli.receive_text_mfsk",
            return_value=("live decoded", None, None, "DEFAULT"),
        ),
    ):
...
def test_receive_error_file_read_failure():
    with (
        ...
        assert result.exit_code == 1
        mock_secho.assert_any_call(
            "Error reading file 'bad.wav': Read error", fg=typer.colors.RED
        )
...
def test_receive_error_no_signal_to_process():
    with (
        ...
        assert result.exit_code == 1
        mock_secho.assert_any_call("No audio signal to process.", fg=typer.colors.RED)
...
def test_receive_warning_sample_rate_mismatch():
    with (
        patch("cli.typer.secho") as mock_secho,
        patch("cli.sf.read", return_value=(np.ones(1000), 22050)),
...
def test_analyze_success():
    ...
    with (
        ...
        assert result.exit_code == 0
        mock_secho.assert_any_call(
            "Detected Signal Mode: DEFAULT", fg=typer.colors.CYAN
        )
...
def test_play_error_file_read_failure():
    with (
        ...
        assert result.exit_code == 1
        mock_secho.assert_any_call(
            "Error playing file 'bad.wav': Play read error", fg=typer.colors.RED
        )
